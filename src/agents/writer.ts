...
//
// Writer Agent — the final step. Takes the Critic-approved findings and
// turns them into a polished, human-readable report.
//
// CRITICAL DESIGN DECISION: chart data is built PROGRAMMATICALLY from the
// real FinancialDataset, never regenerated by the LLM. If we let the LLM
// "write" the chart's numbers, we'd reopen the exact hallucination risk
// the Planner->DataAgent->Analyst->Critic chain was built to close. The
// LLM's only job here is prose: summary, section text, caveats phrasing.
// The numbers on any chart are always a direct, traceable copy of what
// the Data Agent originally fetched.
...
function buildStructuralCaveats(dataset: FinancialDataset, plan: ResearchPlan): string[] {
  ...
  const periodWord = plan.timeframe.range === 1 ? "period" : "periods";
  caveats.push(
    `This report covers only the last ${plan.timeframe.range} ${plan.timeframe.period} ${periodWord} available and should not be treated as investment advice.`
  );
